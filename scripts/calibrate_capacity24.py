"""Bounded 24M full-step CPU calibration: two thread counts x two microbatches."""
...
def one(threads,microbatch,backend,warmup=3,steps=10):
 torch.set_num_threads(threads);torch.manual_seed(773);cfg=LatticeConfig.from_json(ROOT/"configs/capacity24_co4.json");model=build_model(cfg)
 if model.parameter_breakdown()["total"]!=24_097_200:raise RuntimeError("parameter audit")
...
def main():
 ...
 viable=[x for x in rows if x["finite"]];best=max(viable,key=lambda x:x["tokens_per_second"]);result={"rows":rows,"selected":best,"parameter_count":24_097_200,"status":"PASS"};a.output.write_text(json.dumps(result,indent=2)+"\n");print(json.dumps(result))
