"""Exact-resume 24.10M Co4 capacity training on certified DATA-D-v3."""
...
ROOT=Path(__file__).resolve().parents[1];TOK=ROOT/"artifacts/tokenizers/babylm_2026_4k.json";CONFIG=ROOT/"configs/capacity24_co4.json";SOURCES=("fineweb_edu","wikipedia","fineweb");EXPECTED=24_097_200;STOP=False
...
def main():
 ...
 p=argparse.ArgumentParser();p.add_argument("--manifest",type=Path,required=True);p.add_argument("--target",type=int,required=True,choices=(50_000_000,100_000_000,150_000_000));p.add_argument("--run-id",default="co4-capacity24-data-d-v3");p.add_argument("--backend",choices=("fp32_eager","fp32_compile"),required=True);p.add_argument("--threads",type=int,required=True);p.add_argument("--microbatch",type=int,choices=(4,8),default=8);p.add_argument("--fresh",action="store_true");p.add_argument("--resume",action="store_true");p.add_argument("--hard-deadline-epoch",type=float);p.add_argument("--curve",type=Path,default=ROOT/"artifacts/capacity24_training_curve.csv");a=p.parse_args()
 ...
 if cert.get("acceptance")!="PASS" or cert.get("manifest_sha256")!=mh or top["total_unique_tokens"]<150_000_000:raise RuntimeError("uncertified or insufficient DATA-D-v3")
 cfg=LatticeConfig.from_json(CONFIG);torch.set_num_threads(a.threads);torch.set_num_interop_threads(1);random.seed(cfg.seed);torch.manual_seed(cfg.seed)
 ...
 if model.parameter_breakdown()["total"]!=EXPECTED:raise RuntimeError("capacity24 parameter identity mismatch")
