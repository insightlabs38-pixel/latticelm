...
def main():
 ...
 torch.set_num_threads(2); started=time.perf_counter(); ck=torch.load(a.checkpoint,map_location='cpu',weights_only=False); c=LatticeConfig(**ck['config']); model=build_model(c); model.load_state_dict(ck['model']); model.eval(); tok=load_tokenizer(a.tokenizer)
 ...
 result={'dataset':'Salesforce/wikitext','config':'wikitext-103-raw-v1','split':'test','tokens_scored':count,'negative_log_likelihood':nll,'perplexity':math.exp(nll/count),'wall_seconds':time.perf_counter()-started,'checkpoint_sha256':digest(a.checkpoint),'tokenizer_sha256':digest(a.tokenizer)}
