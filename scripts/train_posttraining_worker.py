...
def main():
 ...
 while tokens<a.tokens:
  ...
  if a.method in {"sft","recovery"}:
   ...
   if a.method=="joint":
    item=encode_example(tok,ex,cfg.context_length);completion,extra,full=batch_completion(model,[item]);processed_tokens+=full;increment+=extra;loss=joint_objective(completion,rank,rank_weight=a.rank_weight)
   else:loss=rank
  elif a.method=="simpo":
   ...
   candidates=[tok.encode(" "+ex.answer),tok.encode(" "+next(x for x in ex.candidates if x!=ex.answer))];_,scores=batch_candidate_scores(model,tok.encode(ex.prompt),candidates,cfg.context_length);loss=simpo_loss(scores[:1],scores[1:]);increment=len(candidates[0]);processed_tokens+=sum(min(cfg.context_length,len(tok.encode(ex.prompt))+len(c)) for c in candidates)
  else:
