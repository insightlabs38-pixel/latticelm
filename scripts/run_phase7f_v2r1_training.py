"""Train the controlled Co4-L DATA-D-BROAD-v2r1 lineage to exactly 25M tokens."""
from __future__ import annotations

import argparse, csv, json, math, random, resource, time
...
SOURCES=("fineweb_edu","wikipedia","fineweb");TARGET=25_000_000
...
def main():
 p=argparse.ArgumentParser();p.add_argument("--manifest",required=True);p.add_argument("--fresh",action="store_true");p.add_argument("--resume",action="store_true");p.add_argument("--stop-tokens",type=int);a=p.parse_args()
 if a.fresh==a.resume:p.error("choose exactly one of --fresh or --resume")
 target=a.stop_tokens or TARGET
 if target>TARGET:raise RuntimeError("Phase 7F must not continue past 25M")
 ...
 milestones={1_000_000,3_000_000,5_000_000,7_500_000,10_000_000,15_000_000,20_000_000,25_000_000};evaluated={x for x in milestones if x<=tokens}
 ...
 while tokens<target:
  ...
  crossed=[m for m in milestones-evaluated if abs(tokens-m)<=512];evaluation=tokens==target or bool(crossed);periodic=step%config.checkpoint_interval==0
 ...
 if tokens!=target or (target==TARGET and source_tokens!={"fineweb_edu":12_500_000,"wikipedia":6_250_000,"fineweb":6_250_000}):raise RuntimeError("exact terminal mixture failure")
