...
def main():
 p=argparse.ArgumentParser();p.add_argument("--output",type=Path,required=True);a=p.parse_args();root=a.output.parent/"capacity24-live-drill-checkpoints";cfg=LatticeConfig(architecture="co4_causal",vocab_size=64,d_model=32,n_layers=2,n_heads=4,n_kv_heads=1,ffn_hidden=96,context_length=16,batch_size=8,memory_enabled=False,tie_embeddings=False,seed=91)
