...
def run_worker(args):
 checkpoint,tokenizer,threads,seed=args
 import torch
 ...
 from latticelm.posttraining.rollout import sample
 ...
 for i in range(2):
  ex=generate_example(seed+i,split="validation");t=sample(model,tok.encode(ex.prompt)[-240:],ex.global_id,lambda ids:verify_candidate(ex,tok.decode(list(ids)).strip()),max_new_tokens=8,seed=seed+i,policy_sha256="topology-only");tokens+=len(t.generated_ids)
...
def main():
 ...
 rows=[]
 for processes,threads in ((1,16),(2,8),(4,4),(8,2)):
  started=time.perf_counter()
  with ProcessPoolExecutor(max_workers=processes) as pool:values=list(pool.map(run_worker,[(a.checkpoint,a.tokenizer,threads,1000+i*10) for i in range(processes)]))
  wall=time.perf_counter()-started;rows.append({"processes":processes,"threads":threads,"verified_rollout_tokens":sum(x["tokens"] for x in values),"wall_seconds":wall,"aggregate_tokens_per_second":sum(x["tokens"] for x in values)/wall,"aggregate_peak_rss_bytes":sum(x["rss_bytes"] for x in values),"stable":True})
 winner=max(rows,key=lambda x:x["aggregate_tokens_per_second"]);result={"schema":"rollout-backend-certification-v1","status":"PASS","trajectory_parity":cert,"topologies":rows,"selected":winner};a.output.parent.mkdir(parents=True,exist_ok=True);a.output.write_text(json.dumps(result,indent=2)+"\n");print(json.dumps(result))
