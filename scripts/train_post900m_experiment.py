...
def train_branch(base:Path,mode:str,cid:str,added:int,lr:float)->dict:
    state=torch.load(base,map_location="cpu",weights_only=False);cfg=LatticeConfig(**state["config"]);model=build_model(cfg);model.load_state_dict(state["model"],strict=True);model.compile(mode="max-autotune-no-cudagraphs",fullgraph=False);mix,vals=data_setup(cfg);tok=load_tokenizer(TOK)
    opt=torch.optim.AdamW(model.parameters(),lr=lr,weight_decay=cfg.weight_decay,betas=(cfg.adam_beta1,cfg.adam_beta2),eps=1e-8)
    if mode=="wsd":opt.load_state_dict(state["optimizer"]);mix.load_state_dict(state["data_source_selector_state"])
    total=max(1,math.ceil(added/1024));start=time.perf_counter();last=float("nan")
    consumed=0
    for step in range(total):
        if STOP:raise InterruptedError("safe stop")
        ...
        loss.backward();torch.nn.utils.clip_grad_norm_(model.parameters(),cfg.grad_clip);opt.step();last=float(loss);consumed+=increment
    payload={**state,"model":model._orig_mod.state_dict() if hasattr(model,"_orig_mod") else model.state_dict(),"optimizer":opt.state_dict(),"lineage":cid,"run_id":cid,"parent_checkpoint_sha256":sha256_file(base),"branch_kind":mode,"branch_training_tokens":consumed,"tokens_seen":int(state["tokens_seen"])+consumed,"learning_rate_policy":"cosine-to-zero" if mode=="wsd" else "constant-low-lr","wall_seconds":time.perf_counter()-start,"train_loss":last,"python_rng_state":random.getstate(),"numpy_rng_state":np.random.get_state(),"torch_rng_state":torch.get_rng_state()}
    if mode=="wsd":payload["data_source_selector_state"]=mix.state_dict();payload["next_batch_sha256"]=digest_next(mix)
    cp=ART/"checkpoints"/cid/"milestone.pt";immutable(cp,payload);reason=reasoning_accuracy(model,tok) if mode=="sft" else 0.0;return eval_all(cp,cid,validation(model,vals),reason)
...
def main():
    ...
    if a.mode=="wsd":specs=((f"{a.base_id.lower()}-wsd-short",25_000_000,3e-4),(f"{a.base_id.lower()}-wsd-long",50_000_000,3e-4))
