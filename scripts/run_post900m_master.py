...
def write_handoff(s,status,terminal,reason=None,metrics=None,classification=None):
    cert=json.loads(CERT.read_text()) if CERT.exists() else {};obj={"master":"post900m","status":status,"terminal_stage":terminal,"decision_required":"human_review" if status!="COMPLETE" else "evaluate_1_5B_scaling","base_checkpoint":{"tokens":s.get("final_tokens",TARGET),"sha256":s.get("final_checkpoint_sha256",cert.get("checkpoint_sha256"))},"predecessor_900m":{"sha256":cert.get("checkpoint_sha256")},"data":s.get("data",{"version":"DATA-D-v3","manifest_sha256":cert.get("manifest_sha256"),"certification":cert.get("status")}),"metrics":metrics or s.get("metrics",{}),"scaling":{"classification":classification or s.get("classification"),"projected_2b_gain":s.get("projected_2b_gain")},"reason":reason,"next_allowed_actions":["EVALUATE_2B_CONTINUATION","RUN_WSD_TOURNAMENT","RUN_POSTTRAINING_TOURNAMENT","RUN_SYSTEMS_OPTIMIZATION"],"evidence_files":[evidence_name(p) for p in (CERT,SCALING,DECISION,REPORT) if p.exists()],"generated_at":now()};atomic(HANDOFF,obj)
def report(s,classification,cert,wiki,gibc):
    ...
    REPORT.write_text(text);return metrics
def execute(s,once=False,dry_run=False):
    ...
    if classification=="SATURATING_OR_LOW_VALUE":
        # A bounded tournament is a separate lineage. Never silently mutate BASE_900M.
        atomic(ART/"post900m_saturation_response_plan.json",{"status":"REQUIRES_IMPLEMENTED_EXPERIMENT_RUNNERS","base_checkpoint_sha256":cert["checkpoint_sha256"],"ordered_phases":["WSD_COOLDOWN","SYSTEMS_BENCHMARK","SFT","RLVR_GRPO","OPTIONAL_DPO"],"constraints":["immutable branches","deterministic supervision only","no external model judge","official evaluation required for promotion"]})
        block(s,"saturation response experiment runners are not installed; expensive autonomous experiments are not safely authorized",kind="BLOCKED");return
