...
from latticelm.lattice_reason_v2.dataset import build,verify_manifest
...
def test_reservoir_tokenizer_identity_and_context_cap(tmp_path):
 from tokenizers import Tokenizer
 from tokenizers.models import WordLevel
 from tokenizers.pre_tokenizers import Whitespace
 tokenizer=Tokenizer(WordLevel({"[UNK]":0,"Answer":1},unk_token="[UNK]"));tokenizer.pre_tokenizer=Whitespace();path=tmp_path/"tokenizer.json";tokenizer.save(str(path));manifest=build(tmp_path/"pool",path,100,shard_tokens=100,reserve_bytes=0);verified=verify_manifest(tmp_path/"pool/manifest.json",path);assert manifest["context_limit"]==256 and verified["tokenizer_sha256"]==manifest["tokenizer_sha256"]
 bad=tmp_path/"bad.json";bad.write_text("{}")
 import pytest
 with pytest.raises(ValueError):verify_manifest(tmp_path/"pool/manifest.json",bad)
