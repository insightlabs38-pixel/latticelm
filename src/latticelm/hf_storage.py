...
def export_checkpoint(
    ...
    manifest = {
        ...
        "training_dataset": dataset_manifest.get("dataset", dataset_manifest.get("corpus_identity")),
        "dataset_revision": dataset_manifest.get("revision", dataset_manifest.get("builder_git_commit")),
        "dataset_manifest_sha256": sha256(dataset_manifest_path),
        "dataset_sources": dataset_manifest.get("sources", list(dataset_manifest.get("mixture_definition", {}))),
        "training_tokens_seen": int(metrics["tokens_trained"]),
        "optimizer": metrics.get("optimizer", checkpoint.get("optimizer_kind", config.optimizer)),
        "optimizer_settings": {"learning_rate": metrics.get("learning_rate", checkpoint.get("learning_rate", config.learning_rate)),
                               "weight_decay": config.weight_decay, "replay_fraction": metrics.get("replay_fraction", checkpoint.get("replay_fraction")),
